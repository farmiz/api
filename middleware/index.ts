...
class AuthMiddleware {
  ...
  public async isPermitted(
    ...
    try {
      ...
      return res
        ...
        .json({ error: true, response: { message: ERROR_MESSAGES.permissionDenied } });
    } catch (error: any) {
      return res
        .status(httpCodes.FORBIDDEN.code)
        .json({ error: true, response: { message: ERROR_MESSAGES.accessDenied } });
    }
  }
  public async rateLimit(
    ...
    try {
      ...
      const rateLimiterRes = await rateLimiterMongo.consume(
        `${config.name}:${ip}:${req.method.toLowerCase()}`,
      );
      const headers = {
  ...
  async getServerIp(req: AuthRequest, res: Response, next: NextFunction) {
    ...
    if (ipv4Address && ipv4Address === "127.0.0.1") {
      ipv4Address = "185.212.71.137";
    }

    const geolocation = geoLocation.lookup(ipv4Address);

    if (geolocation && BANNED_COUNTRIES[geolocation.country]) {
      next(
        new RequestError(
...
export function isAsyncFunction(func: Function | Promise<Function> | RequestHandler | Promise<RequestHandler>) {
